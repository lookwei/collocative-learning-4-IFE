...
class CAG_Module(nn.Module):
    ...
    def forward(self, x):
        y_sig = torch.exp(-(self.coordinate - self.weight[0]) ** 2 / (2 * self.weight[1] ** 2)) / (torch.sqrt(self.pi) * self.weight[1])
...
class ResNet(nn.Module):
    def __init__(self, block, layers,  network_type, num_classes, att_type=None, use_mask=[False, False, False, False], input_size=100):
        self.inplanes = 64
    ...
    def _make_layer(self, block, planes, blocks, stride=1, att_type=None, use_mask=False, feature_map_size=100):
        downsample = None
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, use_cbam=att_type=='CBAM', use_mask=use_mask, feature_map_size=feature_map_size))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, use_cbam=att_type=='CBAM', use_mask=use_mask, feature_map_size=feature_map_size))
...
def ResidualNet(network_type, depth, num_classes, att_type, use_mask=[False, False, False, False], input_size=100):
